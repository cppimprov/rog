...
namespace luups
{
	...
	void state_view::register_fns(std::span<lua_reg const> fns, int num_upvalues)
	{
		...
		check_or_error(num_upvalues, "too many upvalues");
...
// 
	...
	void state_view::check_or_error(int num_elements, std::string const& msg)
	{
		die_if(!is_open());
	...
	void state_view::push_fulluserdata(std::size_t block_size, int num_user_values)
	{
		die_if(!is_open());
		check_or_error(1);
		// note: the user should call get_userdata to get the memory block address
	...
	lua_integer state_view::pop_integer()
	{
	...
	lua_number state_view::pop_number()
	{
	...
	std::string state_view::pop_string()
	{
	...
	lua_cfunction state_view::pop_cfunction()
	{
	...
	void* state_view::pop_userdata()
	{
		auto const result = get_userdata();
		pop();
		return result;
	}

	bool state_view::get_boolean(int index) const
	{
		die_if(!is_open());
	...
	lua_integer state_view::get_integer(int index) const
	{
		die_if(!is_open());
	...
	lua_number state_view::get_number(int index) const
	{
		die_if(!is_open());
	...
	std::string_view state_view::get_string(int index) const
	{
		die_if(!is_open());
	...
	lua_cfunction state_view::get_cfunction(int index) const
	{
		die_if(!is_open());
	...
	void* state_view::get_userdata(int index) const
	{
		die_if(!is_open());
		die_if(!is_userdata(index));
		return lua_touserdata(L, index);
	}

	std::optional<bool> state_view::try_get_boolean(int index) const
	{
		if (!is_boolean(index))
			return { };
	...
	std::optional<lua_integer> state_view::try_get_integer(int index) const
	{
		if (!is_integer(index))
			return { };
	...
	std::optional<lua_number> state_view::try_get_number(int index) const
	{
		if (!is_number(index))
			return { };
	...
	std::optional<std::string_view> state_view::try_get_string(int index) const
	{
		if (!is_string(index))
			return { };
	...
	std::optional<lua_cfunction> state_view::try_get_cfunction(int index) const
	{
	...
	std::optional<void*> state_view::try_get_userdata(int index) const
	{
	...
	bool state_view::get_boolean_or(bool default_value, int index) const
	{
		return is_boolean(index) ? to_boolean_impl(index) : default_value;
	}

	lua_integer state_view::get_integer_or(lua_integer default_value, int index) const
	{
		return is_integer(index) ? to_integer_impl(index) : default_value;
	}

	lua_number state_view::get_number_or(lua_number default_value, int index) const
	{
		return is_number(index) ? to_number_impl(index) : default_value;
	}

	std::string_view state_view::get_string_or(std::string_view default_value, int index) const
	{
		return is_string(index) ? to_string_impl(index) : default_value;
	}

	lua_cfunction state_view::get_cfunction_or(lua_cfunction default_value, int index) const
	{
		return is_cfunction(index) ? to_cfunction_impl(index) : default_value;
	}

	void* state_view::get_userdata_or(void* default_value, int index) const
	{
		return is_userdata(index) ? lua_touserdata(L, index) : default_value;
	}
	...
	bool state_view::to_boolean_impl(int index) const
	{
		// n.b. assumes state is valid and type is boolean
		return lua_toboolean(L, index);
	}

	lua_integer state_view::to_integer_impl(int index) const
	{
		// n.b. assumes state is valid and type is number *and* integer
		return lua_tointeger(L, index);
	}

	lua_number state_view::to_number_impl(int index) const
	{
		// n.b. assumes state is valid and type is number
		return lua_tonumber(L, index);
	}

	std::string_view state_view::to_string_impl(int index) const
	{
	...
	lua_cfunction state_view::to_cfunction_impl(int index) const
	{
