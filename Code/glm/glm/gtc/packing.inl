...
//	}
	...
	struct compute_half<3, Q>
	{
		GLM_FUNC_QUALIFIER static vec<3, uint16, Q> pack(vec<3, float, Q> const& v)
		{
			...
			u16vec3 Packed;
			memcpy((void*)&Packed, &Unpack, sizeof(Packed));
			return Packed;
		}
		...
		{
			i16vec3 Unpack;
			memcpy((void*)&Unpack, &v, sizeof(Unpack));
	...
	{
		u8vec2 Unpack;
		memcpy((void*)&Unpack, &p, sizeof(Unpack));
	...
	GLM_FUNC_QUALIFIER uint16 packSnorm2x8(vec2 const& v)
	{
		...
		uint16 Packed = 0;
		memcpy((void*)&Packed, &Topack, sizeof(Packed));
		return Packed;
	}
	...
	{
		uint64 Pack = 0;
		memcpy((void*)&Pack, &v, sizeof(Pack));
		return Pack;
	}
