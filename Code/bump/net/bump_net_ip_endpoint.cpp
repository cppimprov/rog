...
namespace bump
{
	...
	namespace net
	{
		...
		namespace ip
		{
			...
			{
				auto const family = get_address_family();
				if (family == address_family::V4) return to_system_byte_order(get_address_v4().sin_port);
				if (family == address_family::V6) return to_system_byte_order(get_address_v6().sin6_port);
				die(); // invalid address family
			}
