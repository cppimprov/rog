...
class PlatformMSVC:
	...
	def write(self, n, build_type):
		...
		lualib.src_files = [ join_file(lualib.code_dir, f) for f in lualib.src_files ]
		self.write_static_lib(n, build_type, lualib, '2', '/TP')

		lua = ProjectExe.from_name('lua', self, build_type)
		lua.defines = lualib.defines
		lua.src_files = lualib.src_files + [ join_file(lua.code_dir, 'lua.c') ]
		...
		luac.src_files = lualib.src_files + [ join_file(luac.code_dir, 'luac.c') ]
